...
public class ImageToTextServiceImpl implements ImageToTextService {
    ...
    private String callOriginalApi(String imageBase64, String prompt) {
        try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            finalPrompt += "。要求：只使用中文，不要生成任何英文单词、字母或英文表达，用中文表达亲昵如\"亲爱的\"、\"宝贝\"等，但不要使用\"么么哒\"这样的表达。";
    ...
    private String callOriginalApiTextOnly(String prompt) {
        ...
        try {
            ...
            // 构建请求体（不包含图片）- 添加中文限制
            String finalPrompt = prompt + "。要求：只使用中文，不要生成任何英文单词、字母或英文表达，用中文表达亲昵如\"亲爱的\"、\"宝贝\"等，但不要使用\"么么哒\"这样的表达。";
