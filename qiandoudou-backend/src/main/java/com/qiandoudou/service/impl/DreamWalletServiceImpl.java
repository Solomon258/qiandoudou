    ...
    public String generateTravelItinerary(String destination, Integer days, BigDecimal budget) {
        ...
        try {
            ...
            String prompt = String.format(
                ...
                "4. 住宿建议\n" +
                "5. 特色体验活动\n" +
                "6. 根据目的地和天数，给出合理的预算建议（包含交通、住宿、餐饮、门票等）\n\n" +
                "格式要求：\n" +
                "- 标题使用 Markdown 格式：主标题用 # （一个井号），每天的标题用 #### （四个井号）\n" +
                "- 主标题格式：# %s7日深度游行程规划\n" +
                "- 每日标题格式：#### Day1: 标题内容\n" +
                "- 列表使用 - 开头\n" +
                "- 在最后必须添加预算总计，格式为：**总计**：人均约XXXX-XXXX元（XXXX为具体数字）\n\n" +
                "请用清晰的 Markdown 格式输出，每天的行程分段描述，让人一目了然。",
                destination, destination, days, destination
            );
